...
<html>
<head>
<link rel="stylesheet" type="text/css" href="css/main_admin.css?ver=1">
...
<div class="memberHelpMain">
	...
	<div class="box">
		<ul class="memberHelpSerchBarFeild">
			...
			<li class="menuli"><a style="${backD_color}"class="a" href="adminEmailType.do?hqt_type=이벤트" onclick="addType3()">이벤트</a></li>
			<li class="menuli"><a style="${backE_color}"class="a" href="adminEmailType.do?hqt_type=분실" onclick="addType4()">분실</a></li>
	...
	<div>
		...
		<c:forEach var="dto" items="${list }">
			<c:url var="emailHelpUrl" value="adminEmailBorder.do">
					<c:param name="idx">
					${dto.hqt_idx }
					</c:param>
				</c:url>
			<tr>
				<td>${dto.hqt_type }</td><td class="borderurl"><a href="${emailHelpUrl }">${dto.hqt_subject }</a></td><td>${dto.hqt_region }</td><td>${dto.hqt_cinema }</td>
			</tr>
		</c:forEach>
			</tbody>
			<tfoot>
